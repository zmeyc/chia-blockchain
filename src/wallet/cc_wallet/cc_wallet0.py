...
class CCWallet:
    ...
    async def create_new_cc(
        ...
        regular_record = TransactionRecord(
            ...
            to_puzzle_hash=Program(Program.to((1))).get_tree_hash(),
    ...
    async def generate_new_coloured_coin(self, amount: uint64) -> SpendBundle:
        ...
        origin = coins.copy().pop()
        origin_id = origin.name()
        prog = load_clvm("long_string.clvm", package_or_requirement=__name__)
        cc_inner_hash = prog.get_tree_hash()
        ...
        list_of_solutions = [CoinSolution(coin, full_solution)]
        child_spend = SpendBundle(list_of_solutions, G2Element.infinity())
        spend_bundle = child_spend.aggregate([tx_record.spend_bundle, child_spend])
