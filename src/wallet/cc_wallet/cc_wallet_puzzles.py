...
from src.wallet.puzzles.load_clvm import load_clvm1
...
def cc_make_core(originID):
    ...
    s = load_clvm1("coloured_coins.clvm")
    s1 = binutils.disassemble(s)
    genesis_id = str(originID)
    s2 = s1.replace('"REPLACE_ME_WITH_GENESIS_ID"', "0x%s" % genesis_id)
    return s2
...
def cc_make_solution(
    ...
    r = Program(binutils.assemble(sol))
    print(r)
    breakpoint()
    return r
