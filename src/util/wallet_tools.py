...
class WalletTool:
    ...
    def make_solution(self, condition_dic: Dict[ConditionOpcode, List[ConditionVarList]]) -> Program:
        ret = []
    ...
    def generate_unsigned_transaction(
        ...
        output = ConditionVarList(ConditionOpcode.CREATE_COIN, newpuzzlehash, int_to_bytes(amount))
        condition_dic[output.opcode].append(output)
        amount_total = sum(int_from_bytes(cvl.vars[1]) for cvl in condition_dic[ConditionOpcode.CREATE_COIN])
        change = spend_value - amount_total - fee
        if change > 0:
            changepuzzlehash = self.get_new_puzzlehash()
            change_output = ConditionVarList(ConditionOpcode.CREATE_COIN, changepuzzlehash, int_to_bytes(change))
